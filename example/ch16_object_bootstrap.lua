--[[
    Chapter 16: Forward declarations and System.Object bootstrap.
]]
...
do
    ...
    if currentModule ~= nil then
        local bRet, result = pcall(require, currentModule .. "_config")
        if bRet then
...
local function createEmptyType(typeName, forwardDeclare)
    local resultTable
...
local function createType(baseTypeTable, typeNameOrForwardDeclare, bInterface)
    ...
    if type(typeNameOrForwardDeclare) == "table" then
        ...
        if forwardDeclareMeta == nil then
            error("invalid forward declare (type table expected, got " .. type(typeNameOrForwardDeclare) .. ")", 2)
        end
    ...
    local theType = createEmptyType(typeName, theForwardDeclare)
    local typeMeta = {}
    ...
    if baseTypeTable ~= nil then
        ...
        if baseTypeTable ~= Lplus.Object and isBaseInterface ~= bInterface then
            if bInterface then
        ...
        return createProxy
        {
            ...
            __newindex = function(_, fieldName, initValue)
                ...
                if feature_member_info then
                    ...
                    memberInfo = createMemberInfo("field", style, theType, true, fieldType)
                end
    ...
    local function commit()
        ...
        if not bInterface and feature_declare_checking then
        ...
        if feature_accessing_checking then
            ...
            function typeMeta.__index(_, memberName)
                checkMemberInfo(memberName, false, 2)
            ...
            function typeMeta.__call(_, ...)
                ...
                for fieldName, initValue in pairs(typeMeta.fields) do
                    ...
                    assignInstanceMember(fieldName, realInitValue, 2)
                end
...
local function typeTableToTypeInfo(typeTable)
    ...
    if typeInfo == nil then
        if Lplus.Type ~= nil and Lplus.Type.fromTypeTable ~= nil then
            ...
            typeInfo =
            {
                ...
                toTypeTable = function(self)
                    return self.m_typeTable
                end,
            }
            setmetatable(typeInfo,
            {
                typeTable = createForwardDeclareType("System.Type", 1),
                magic = lplusObjectMagic,
            })
        end
...
do
    ...
    def.virtual("=>", "string").toString = function(self)
        local typeInfo = self:getType()
        return ("%s#%s"):format(typeInfo:getName(), tostring(self:getTypeTable()))
    end
